...
**Goal:** Make every task in the business-orchestrator dashboard show full structured logs (requests, responses, status transitions, errors) per task — matching the agentic-email-processing-system pattern where each task/email has a scrollable log timeline.
...
```bash
grep -n "post\|baseURL" /home/ssf/Documents/Github/business-orchestrator/src/common/logging/logging.client.ts
```
...
curl -s -X POST http://localhost:3367/api/logs \
  ...
  -d '{"service":"business-orchestrator","level":"info","message":"test-log-path-fix","task_id":"test-123","timestamp":"2026-05-16T00:00:00.000Z","duration_ms":0}' | jq .
```
...
Then query back:
```bash
curl -s "http://localhost:3367/api/logs/query?service=business-orchestrator&task_id=test-123" | jq '.data | length'
```
...
```bash
cd /home/ssf/Documents/Github/business-orchestrator && ./scripts/deploy.sh
```
...
```bash
grep -n "logger.log" /home/ssf/Documents/Github/business-orchestrator/src/coding-worker/coding-worker-agent.service.ts | grep -v "taskId"
```
...
```bash
grep -n "logger.log\|task\.id\|taskId" /home/ssf/Documents/Github/business-orchestrator/src/coordinator/project-coordinator.service.ts | head -40
```
...
```bash
grep -n "task.updated\|socket.*on\|liveSocket" /home/ssf/Documents/Github/business-orchestrator/public/app.js | head -20
```
