...
The `.optional()` is redundant — `.default()` already handles missing fields. runlayer does not have `.optional()` here. Removing it makes the schemas identical on this field.

**File:** `runlayer/src/contracts/ai-complete.contract.ts`
...
### 8. Fix `runlayer` missing output gates
...
Add `NotificationsClient` injection and fire-and-forget escalation — mirror `runlayer`'s implementation exactly.
...
| Feature | runlayer | ai-microservice |
|---------|----------------------|-----------------|
