# TASKS.md

## Active IPS-governed tasks

None. New implementation work must start with an approved IPS task, goal-impact record, execution plan, and validation plan before runtime code changes.

## Backlog

New implementation work must start with an IPS task, goal-impact record, execution plan, and validation plan before runtime code changes.

## Completed

- 2026-06-19 `TASK-001` Intent Preservation System adoption completed and validated. Artifacts: `11_tasks/TASK-001-implement-ips-standard.md`, `21_execution_plans/EP-TASK-001-implement-ips-standard.md`, `22_goal_impact/GOAL-IMPACT-TASK-001.md`, `12_validation/VAL-TASK-001-ips-adoption.md`.

## Project Completion Marker

- 2026-06-21: Project marked completed/frozen after remote inventory. There are no active goals, active plans, open tasks, blockers, or pending human/AI actions. Do not ask for a new goal during routine status checks unless the owner explicitly creates one.
