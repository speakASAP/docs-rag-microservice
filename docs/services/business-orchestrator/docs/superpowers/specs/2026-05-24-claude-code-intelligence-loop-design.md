...
Add Claude Code (CC) as an active intelligence layer inside `business-orchestrator`, operating in two roles:
...
- No new microservice. Everything lives inside `business-orchestrator`.
...
Body:
  ...
  ## Links
  - Trace: https://orchestrator.alfares.cz/dashboard/goals/<goalId>
...
| `GITHUB_REPO` | _(required)_ | Repo slug for gh CLI calls, e.g. `speakASAP/business-orchestrator` |
...
| Separate microservice? | No — both services live inside business-orchestrator |
