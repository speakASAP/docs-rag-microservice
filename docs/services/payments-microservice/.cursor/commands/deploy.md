...
Our application payment.alfares.cz consists of several microservices in ~/Documents/Github/ (logging, nginx, database, notifications). Access via ssh alfares.

ssh alfares "cd payments-microservice && git pull"

pull github repos using ssh alfares "cd payments-microservice && git pull && cd ../nginx-microservice && git pull && docker exec nginx-microservice nginx -t && docker exec nginx-microservice nginx -s reload"
...
Run: ssh alfares "cd nginx-microservice && ./scripts/blue-green/deploy.sh payments-microservice"
