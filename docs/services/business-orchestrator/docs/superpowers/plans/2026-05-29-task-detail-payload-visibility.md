...
```bash
grep -n "step-log-meta-block\|step-log-rich-label" /home/ssf/Documents/Github/business-orchestrator/public/style.css
```
...
Navigate to a task detail URL like `https://orchestrator.alfares.cz/tasks/<uuid>`.
...
```bash
grep -n "aiRequestLog\|aiResponseLog" /home/ssf/Documents/Github/business-orchestrator/src/contracts/http-responses.contract.ts
```
...
```bash
grep -n "aiRequestLog\|aiResponseLog" /home/ssf/Documents/Github/business-orchestrator/src/dashboard/dashboard.controller.ts
```
...
Visit `https://orchestrator.alfares.cz/tasks/<uuid>` for a task that has been executed (so `aiRequestLog` and `aiResponseLog` are populated).
