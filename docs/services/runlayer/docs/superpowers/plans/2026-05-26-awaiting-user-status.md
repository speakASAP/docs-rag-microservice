...
**GitHub Issue:** https://github.com/speakASAP/runlayer/issues/18
...
```bash
grep -n "save\|findOne\|async " /home/ssf/Documents/Github/runlayer/src/projects/projects.service.ts | head -20
```
...
```bash
sed -n '1,130p' /home/ssf/Documents/Github/runlayer/src/worker/worker-pool.service.ts
```
...
```bash
grep -n "aiHttp\|complete\|outputRef\|runResult" /home/ssf/Documents/Github/runlayer/src/worker/worker-agent.service.ts | head -20
```
...
- `ALTER TABLE runlayer.projects ADD COLUMN execution_mode VARCHAR DEFAULT 'auto'`
- `ALTER TABLE runlayer.tasks ADD COLUMN pending_question TEXT`
- `ALTER TABLE runlayer.tasks ADD COLUMN ai_request_log JSONB`
- `ALTER TABLE runlayer.tasks ADD COLUMN ai_response_log JSONB`
...
curl -s -X POST https://runlayer.alfares.cz/api/dashboard/projects/<PROJECT_ID>/execution-mode \
...
```bash
gh issue comment 18 --repo speakASAP/runlayer --body "## Completed
...
gh issue close 18 --repo speakASAP/runlayer
```
