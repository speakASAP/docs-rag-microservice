...
### Centralized Authentication via Auth-Microservice

StateX User Portal uses the centralized `auth-microservice` (`https://auth.alfares.cz`) for all authentication operations.

### Authentication Systems

1. **Email/Password Authentication**: Traditional email and password registration/login
2. **Contact-Based Registration**: Registration using contact information (email/phone) without password
   - Used for form submissions and user identification
   - Session-based authentication for contact-based users

### JWT Token Strategy (via auth-microservice)

1. **Access Token**: Generated by auth-microservice (default: 7 days, configurable)
2. **Refresh Token**: Generated by auth-microservice (default: 30 days, configurable)
3. **Token Storage**: HttpOnly cookies for web, localStorage for mobile
4. **Token Validation**: All tokens validated via auth-microservice

### Authentication API Endpoints

**Authentication Endpoints** (delegated to auth-microservice):

- `POST /api/users/register` - Contact-based registration (via auth-microservice)
- `POST /api/users/register-simple` - Simple contact registration (via auth-microservice)
- `POST /auth/login` - Email/password login (via auth-microservice)
- `POST /auth/register` - Email/password registration (via auth-microservice)
- `POST /auth/validate` - Token validation (via auth-microservice)
- `POST /auth/refresh` - Token refresh (via auth-microservice)
...
# or
AI_SERVICE_URL=http://ai-microservice:3380  # Docker network

# Security
ALLOWED_HOSTS=localhost,127.0.0.1,portal.alfares.cz
CORS_ALLOWED_ORIGINS=http://localhost:${FRONTEND_PORT:-3602},https://alfares.cz
...
services:
  user-portal:
    ...
    ports:
      - "${USER_PORTAL_PORT:-3606}:8000"  # Port configured in statex/.env
    environment:
