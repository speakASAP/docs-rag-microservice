...
description: When implementing business-orchestrator features, always write Cursor agent files with Claude as validator sync gates — never write the implementation code directly
type: feedback
originSessionId: cc3ed17b-9cdc-4da7-9220-27fe04bd8539
---
When implementing features for business-orchestrator, act as the Lead Orchestrator agent (`master-prompt.md`):
...
**How to apply:** Any time implementing a feature in business-orchestrator — create agent files, not code. Split into AGENT{N} (implementation) + AGENT{N}V (validator), with Claude running the validator as the sync gate.
