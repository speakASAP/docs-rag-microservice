...
**Goal:** Refactor `CcPlannerService` and `GoalReviewService` to call `AiHttpClient` instead of invoking CC CLI directly, so all AI inference in runlayer routes through `ai-microservice POST /ai/complete`.
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx jest cc-planner --no-coverage 2>&1 | tail -20
```
...
{"new_tasks":[{"type":"notify","idempotency_key":"notify-done","payload_ref":{"message":"Goal completed"},"acceptance_criteria":[],"priority":1,"max_attempts":1,"target_service":"runlayer","smoke_test_urls":[]}],"state_patch":{},"decisions":["Goal reached 100% with no open tasks; emitting notify task."]}
...
const mockConfig = {
  get: jest.fn((key: string) => {
    const map: Record<string, unknown> = {
      'ccAgent.githubRepo': 'speakASAP/runlayer',
    };
    return map[key];
  }),
};
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx jest goal-review --no-coverage 2>&1 | tail -20
```
...
export class GoalReviewService {
  ...
  private defaultTemplate(): string {
    return `You are reviewing a completed goal in the runlayer.
Goal: {{GOAL_TITLE}}
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx jest --no-coverage 2>&1 | tail -30
```
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx tsc --noEmit 2>&1 | head -40
```
