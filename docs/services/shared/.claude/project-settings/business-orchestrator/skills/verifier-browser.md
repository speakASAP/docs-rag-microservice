# Browser Verifier — business-orchestrator

Use this skill when verifying frontend changes to the dashboard at `https://orchestrator.alfares.cz`.
...
EMAIL = 'test@example.com'
PASSWORD = '70fdxIwqY7qUg7vXgaWUm/GBPPH5pAYy'
BASE = 'https://orchestrator.alfares.cz'
...
```bash
POD=$(kubectl get pod -n statex-apps -l app=business-orchestrator -o jsonpath='{.items[0].metadata.name}')
