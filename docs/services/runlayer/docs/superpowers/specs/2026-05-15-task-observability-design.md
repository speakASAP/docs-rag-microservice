...
Tasks in Project OS (`runlayer`) are created and immediately stuck as `created` with no indication of why. The user sees a list of tasks with status badges but no answer to: *Why hasn't this started? What did the AI do? What failed?*
...
All structured log entries from runlayer already go to `logging-microservice` via `LoggingClient`. We extend this pattern:

1. **logging-microservice** gains a `task_id` query parameter on `GET /api/logs/query`
2. **runlayer** tags every meaningful log entry with `metadata.task_id`
...
### 2. runlayer — `LoggingClient` tagging
...
### 3. runlayer — `WorkerPoolService` dispatch logging
...
### 4. runlayer — `WorkerAgentService` step logging
...
### 5. runlayer — `ProjectCoordinatorService` step logging
...
This proxies to `logging-microservice/api/logs/query?task_id=<taskId>&service=runlayer&limit=100`. Returns the array of log entries directly. Auth: `JwtGuard` (same as existing dashboard endpoints).
...
### 7. runlayer — agent health endpoint
...
### 8. runlayer — enable-workers admin endpoint
...
WorkerPoolService.dispatch()
  → logger.log({ msg: 'worker_pool_tick', metadata: { idle_workers, disabled_workers, ... } })
      → LoggingClient.log()  (fire-and-forget, 3s timeout)
          → POST logging-microservice/api/logs  (stores to runlayer.log)
...
Dashboard browser
  → GET /api/dashboard/tasks/:taskId/logs
      → DashboardController proxies to logging-microservice/api/logs/query?task_id=:taskId
          → LogsService reads runlayer.log, filters metadata.task_id === taskId
...
```sql
UPDATE runlayer.agents SET status = 'idle' WHERE type = 'worker' AND status = 'disabled';
```
...
| runlayer | `src/common/logging/logging.client.ts` | Ensure `task_id` sent at root level |
| runlayer | `src/worker/worker-pool.service.ts` | Replace console.log with structured logger |
| runlayer | `src/worker/worker-agent.service.ts` | Add step log entries with full prompt/response in metadata |
| runlayer | `src/coordinator/project-coordinator.service.ts` | Add cycle step log entries with full prompt |
| runlayer | `src/dashboard/dashboard.controller.ts` | Add 3 endpoints: `/tasks/:id/logs`, `/agent-health`, `/admin/agents/enable-workers` |
| runlayer | `public/app.js` | Agent health banner + task step logs panel + blocked_reason inline |
