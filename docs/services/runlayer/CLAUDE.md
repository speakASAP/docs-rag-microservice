...
**Stack**: NestJS · PostgreSQL (`runlayer` schema) · Redis · RabbitMQ · LiteLLM
...
| database-server | `db-server-postgres:5432` | PostgreSQL `runlayer` schema |
...
| `GITHUB_REPO` | Repo slug for `gh` CLI calls, e.g. `speakASAP/runlayer` (required for issue/PR/wiki creation) |
