...
```bash
cat /home/ssf/Documents/Github/runlayer/migrations/010_default_execution_mode_manual.sql
```
...
```bash
grep -n "projectsService\|ProjectsService" /home/ssf/Documents/Github/runlayer/src/worker/worker-pool.service.ts
```
...
grep -n "interceptPending\|manual_mode_intercept\|markPendingApproval\|findAll\|manualProjects" \
  /home/ssf/Documents/Github/runlayer/src/worker/worker-pool.service.spec.ts
```
...
kubectl exec -n statex-apps deploy/database-server -- \
  psql -U dbadmin -d runlayer \
  -f /dev/stdin < /home/ssf/Documents/Github/runlayer/migrations/010_default_execution_mode_manual.sql
```

If `kubectl exec` with stdin is unavailable, copy the file first:

```bash
kubectl cp /home/ssf/Documents/Github/runlayer/migrations/010_default_execution_mode_manual.sql \
  statex-apps/$(kubectl get pod -n statex-apps -l app=database-server -o jsonpath='{.items[0].metadata.name}'):/tmp/010.sql

kubectl exec -n statex-apps deploy/database-server -- \
  psql -U dbadmin -d runlayer -f /tmp/010.sql
```
...
```bash
kubectl exec -n statex-apps deploy/database-server -- \
  psql -U dbadmin -d runlayer -c \
  "SELECT execution_mode, count(*) FROM runlayer.projects GROUP BY execution_mode;"
```
...
kubectl exec -n statex-apps deploy/database-server -- \
  ...
  "SELECT column_default FROM information_schema.columns
   WHERE table_schema='runlayer' AND table_name='projects' AND column_name='execution_mode';"
```
...
```bash
curl -s https://runlayer.alfares.cz/health | jq .
```
...
Open https://runlayer.alfares.cz in a browser. Navigate to the project. Verify newly created tasks appear in the "Pending Approval" queue with Approve and Reject buttons visible.
