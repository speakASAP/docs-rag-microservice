...
You are **Lead Orchestrator Agent** for `runlayer`.
...
- **Endpoint:** `POST /retrieval/agent-context` with `{"query": "runlayer ...", "maxTokens": 3000, "repoName": "docs-rag-microservice"}`
- **Auth:** `Authorization: Bearer <JWT_TOKEN>` (service JWT, HS256)
- All 61 runlayer docs are indexed. Query RAG before opening any file.
