...
Scripts for operating, debugging, and smoke-testing the runlayer pipeline.  
All scripts live in `runlayer/scripts/` and load credentials automatically from `.env`.
...
./scripts/orch-project-health.sh runlayer
...
| Requirement | Notes |
|-------------|-------|
| `kubectl` | `db-server-postgres` and `runlayer` must be healthy in `statex-apps` |
| `runlayer/.env` | Provides `ORCHESTRATOR_USER_JWT`, `DB_USER`, `DB_NAME`, `PORT_BLUE/GREEN` |
...
./scripts/orch-runlayer-health.sh
```
...
```bash
./scripts/orch-trigger-cycle.sh flipflop-v1
./scripts/orch-trigger-cycle.sh runlayer
...
./scripts/orch-check-tasks.sh runlayer 5
```
...
```bash
./scripts/orch-smoke-test.sh                      # defaults to runlayer
...
bash runlayer/scripts/orch-apply-flipflop-migrations.sh
```
...
| `_db_query <sql>` | Runs SQL in `db-server-postgres` with schema `runlayer` |
...
docker logs $(docker ps -q --filter name=runlayer-blue 2>/dev/null || \
             docker ps -q --filter name=runlayer-green) --since=10m 2>&1 | \
...
docker exec $(docker ps -q --filter name=runlayer-green) \
...
| `runlayer` | `a9bd9c11-8328-4dc7-8f12-8b228810d3b5` | Self-dogfood project |
