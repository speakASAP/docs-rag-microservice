...
**Goal:** Add Claude Code as a planning agent and post-completion review agent inside runlayer, creating a self-improving loop where every completed goal generates a GitHub issue + optional PR with orchestrator improvements.
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx jest cc-planner.service.spec --no-coverage 2>&1 | tail -20
```
...
```bash
cd /home/ssf/Documents/Github/runlayer && npx jest goal-review.service.spec --no-coverage 2>&1 | tail -20
```
...
**Files:**
- Modify: `CLAUDE.md` (project CLAUDE.md at `/home/ssf/Documents/Github/runlayer/CLAUDE.md`)
...
| `GITHUB_REPO` | Repo slug for gh CLI calls, e.g. `speakASAP/runlayer` |
```
