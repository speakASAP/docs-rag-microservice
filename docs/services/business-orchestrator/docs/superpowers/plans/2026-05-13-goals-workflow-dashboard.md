...
**Tech Stack:** NestJS (TypeScript), TypeORM, PostgreSQL (`business_orchestrator` schema), vanilla JS + HTML dashboard at `public/`
...
ALTER TABLE business_orchestrator.goals
...
DROP INDEX IF EXISTS business_orchestrator.uq_goals_active_per_project;

CREATE UNIQUE INDEX IF NOT EXISTS uq_goals_one_inflight_per_project
  ON business_orchestrator.goals (project_id)
...
```bash
psql postgresql://postgres:postgres@db-server-postgres.statex-apps.svc.cluster.local:5432/postgres \
  -c "SELECT column_name, data_type FROM information_schema.columns WHERE table_schema='business_orchestrator' AND table_name='goals' AND column_name='proposed_plan';"
```
...
@Entity({ name: 'goals', schema: 'business_orchestrator' })
export class Goal {
...
curl -s -H "Authorization: Bearer $TOKEN" https://orchestrator.alfares.cz/api/dashboard/goals | jq 'length'

# Dashboard tasks endpoint
curl -s -H "Authorization: Bearer $TOKEN" https://orchestrator.alfares.cz/api/dashboard/tasks | jq 'length'

# Agents endpoint
curl -s -H "Authorization: Bearer $TOKEN" https://orchestrator.alfares.cz/api/agents | jq 'length'
```
...
PROJECT_ID=$(curl -s -H "Authorization: Bearer $TOKEN" https://orchestrator.alfares.cz/api/dashboard \
  | jq -r '.businesses[0].projects[0].id')

# Create a goal
GOAL=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  https://orchestrator.alfares.cz/api/projects/$PROJECT_ID/goals \
...
curl -s -X PATCH -H "Authorization: Bearer $TOKEN" \
  https://orchestrator.alfares.cz/api/projects/$PROJECT_ID/goals/$GOAL_ID/start-planning | jq '.status'
# Expected: "planning"

# Wait ~10s for coordinator planning cycle, then check proposedPlan
sleep 10
curl -s -H "Authorization: Bearer $TOKEN" \
  https://orchestrator.alfares.cz/api/projects/$PROJECT_ID/goals/$GOAL_ID | jq '.proposedPlan | length'
# Expected: > 0

# Cleanup: cancel/delete the test goal
curl -s -X DELETE -H "Authorization: Bearer $TOKEN" \
  https://orchestrator.alfares.cz/api/projects/$PROJECT_ID/goals/$GOAL_ID
```

- [ ] **Step 5: Smoke test UI**

Open https://orchestrator.alfares.cz in a browser, log in, then:
