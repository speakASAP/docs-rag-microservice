...
`business-orchestrator` must not expose pricing endpoints or pricing business logic.
...
**Image:** `localhost:5000/business-orchestrator:latest`  
**Deploy:** `./scripts/deploy.sh`  
**Logs:** `kubectl logs -n statex-apps -l app=business-orchestrator -f`  
**Restart:** `kubectl rollout restart deployment/business-orchestrator -n statex-apps`
...
| database-server | `db-server-postgres:5432` | PostgreSQL schema `business_orchestrator` |
