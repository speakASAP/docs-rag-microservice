# `runlayer` — Production Architecture Design
...
**Production URL**: `https://runlayer.alfares.cz`
**Docker Network**: `http://runlayer:3390`
...
**Instance:** Shared `database-server` PostgreSQL, database: `runlayer`
...
`service=runlayer`, `correlation_id`, `business_id`, `project_id`, `task_id`, `agent_id`, `duration_ms`, ISO timestamps.
...
**Exchange:** `runlayer` (topic exchange)
  ...
  → RabbitMQ → notifications-microservice
    → Telegram message to owner:
       "⚠️ [severity] flipflop — stalled 6h
        ...
        Dashboard: https://runlayer.alfares.cz/projects/flipflop"
