...
```bash
cd /home/ssf/Documents/Github/business-orchestrator && npm run start:dev 2>&1 &
sleep 5
```
...
```bash
curl -s -X POST https://orchestrator.alfares.cz/api/dashboard/admin/agents/enable-workers \
...
```bash
curl -s https://orchestrator.alfares.cz/api/dashboard/agent-health \
