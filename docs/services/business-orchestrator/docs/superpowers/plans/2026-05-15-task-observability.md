...
**Architecture:** The logging-microservice gains `task_id` and `project_id` query filters. business-orchestrator emits structured step logs (with full AI payloads in metadata) from worker-pool, worker-agent, and coordinator. The dashboard queries logs via an orchestrator proxy and renders them as a step-log timeline per task, plus an agent health banner.
...
The logging-microservice uses `forbidNonWhitelisted: true` and expects field `message`, but business-orchestrator sends `msg` (not `message`) plus extra root fields (`task_id`, `project_id`, etc.) that aren't in the DTO. **All orchestrator logs are currently silently rejected.** Task 1 fixes this first.
...
| business-orchestrator | `src/common/logging/logging.client.ts` | Add `projectId` to POST body root (already has `task_id`) — verify field name is `message` not `msg` |
| business-orchestrator | `src/worker/worker-pool.service.ts` | Replace console.log with structured logger; log agent counts per type/status on every tick |
| business-orchestrator | `src/worker/worker-agent.service.ts` | Add step logs: budget, AI call start/end (full prompt+response in metadata), validation, requeue, done |
| business-orchestrator | `src/coordinator/project-coordinator.service.ts` | Add step logs: cycle start, AI call start/end (full prompt), task created |
| business-orchestrator | `src/dashboard/dashboard.controller.ts` | Add 3 endpoints: task logs proxy, agent-health, enable-workers |
| business-orchestrator | `src/agents/agents.service.ts` | Add `countByTypeAndStatus()` and `enableAllWorkers()` methods |
| business-orchestrator | `public/app.js` | Agent health banner + step logs panel + blocked_reason inline |
...
**Files:**
- Modify: `business-orchestrator/src/agents/agents.service.ts`
...
**Files:**
- Modify: `business-orchestrator/src/dashboard/dashboard.controller.ts`
...
async taskLogs(
  ...
  try {
    const resp = await axios.get(
      `${loggingUrl}/api/logs/query?task_id=${encodeURIComponent(taskId)}&service=business-orchestrator&limit=${lim}`,
      { timeout: 5000 },
    );
...
```bash
cd /home/ssf/Documents/Github/business-orchestrator && npx jest src/dashboard/dashboard.controller.spec.ts --no-coverage 2>&1 | tail -20
```
...
**Files:**
- Modify: `business-orchestrator/src/worker/worker-pool.service.ts`
- Modify: `business-orchestrator/src/worker/worker-pool.service.spec.ts`
...
```bash
cd /home/ssf/Documents/Github/business-orchestrator && npx jest src/worker/worker-pool.service.spec.ts --no-coverage 2>&1 | tail -20
```
...
**Files:**
- Modify: `business-orchestrator/src/worker/worker-agent.service.ts`
...
**Files:**
- Modify: `business-orchestrator/src/coordinator/project-coordinator.service.ts`
...
## Task 8: Deploy business-orchestrator

- [ ] **Step 1: Run full test suite**

```bash
cd /home/ssf/Documents/Github/business-orchestrator && npm test 2>&1 | tail -30
```
...
```bash
cd /home/ssf/Documents/Github/business-orchestrator && ./scripts/deploy.sh 2>&1 | tail -30
```
...
```bash
kubectl get pods -n statex-apps | grep business-orchestrator
...
```bash
TOKEN=$(kubectl exec -n statex-apps deployment/business-orchestrator -c app -- node -e "
...
```bash
kubectl exec -n statex-apps deployment/business-orchestrator -c app -- node -e "
...
c.connect().then(() => c.query('SELECT type, status, count(*) FROM business_orchestrator.agents GROUP BY type, status ORDER BY type, status')).then(r => { console.log(JSON.stringify(r.rows,null,2)); c.end(); }).catch(e => { console.error(e.message); c.end(); });
" 2>&1
```
...
```bash
kubectl logs -n statex-apps deployment/business-orchestrator -c app -f 2>&1 | grep "worker_pool_dispatch\|worker_ai_call_start\|dispatched" | head -5
```
...
**Files:**
- Modify: `business-orchestrator/public/app.js`
- Modify: `business-orchestrator/public/index.html`
...
```bash
kubectl port-forward -n statex-apps svc/business-orchestrator 3390:3390 &
```

Open `https://orchestrator.alfares.cz/` (or `http://localhost:3390`):
...
- [ ] **Step 1: Deploy business-orchestrator with frontend changes**

```bash
cd /home/ssf/Documents/Github/business-orchestrator && ./scripts/deploy.sh 2>&1 | tail -20
```
...
const { Client } = require('pg');
const c = new Client({ host: process.env.DB_HOST, port: process.env.DB_PORT || 5432, user: process.env.DB_USER, password: process.env.DB_PASSWORD, database: process.env.DB_NAME });
c.connect().then(() => c.query('SELECT id FROM business_orchestrator.tasks WHERE status IN (\\'in_progress\\', \\'done\\') ORDER BY created_at DESC LIMIT 1')).then(r => { console.log(r.rows[0]?.id); c.end(); }).catch(e => { console.error(e.message); c.end(); });
" 2>&1
```
...
curl -s "http://localhost:3367/api/logs/query?task_id=${TASK_ID}&service=business-orchestrator&limit=20" | python3 -m json.tool | grep '"message"\|"level"\|"timestamp"' | head -30
```
...
```bash
curl -s "http://localhost:3367/api/logs/query?task_id=${TASK_ID}&service=business-orchestrator&limit=20" | python3 -m json.tool | grep -A2 "worker_ai_call_start"
```
