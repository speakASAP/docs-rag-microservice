...
**Goal:** Align the shared `AiCompleteRequest`/`AiCompleteResponse` contract between `runlayer` and `ai-microservice`, remove dead envelope code, fix the unvalidated health endpoint, and add a CI-runnable parity script to prevent future drift.
...
| Modify | `runlayer/src/contracts/ai-complete.contract.ts` |
...
- [ ] **Step 5: Add missing type exports to runlayer**

In `runlayer/src/contracts/ai-complete.contract.ts`, add two type exports after the existing ones:
...
- [ ] **Step 6: Run runlayer contract tests**

```bash
cd /home/ssf/Documents/Github/runlayer && npx jest --testPathPattern="contracts.spec" --no-coverage 2>&1 | tail -20
```
...
git commit -m "fix(contracts): align AiCompleteRequestSchema with runlayer (remove redundant .optional)"

cd /home/ssf/Documents/Github/runlayer && git add src/contracts/ai-complete.contract.ts
...
FILE_A="$REPO_ROOT/runlayer/src/contracts/ai-complete.contract.ts"
...
if [[ -z "$DIFF" ]]; then
  ...
  echo "" >&2
  echo "--- runlayer/src/contracts/ai-complete.contract.ts" >&2
...
- [ ] **Step 3: Run runlayer full test suite**

```bash
cd /home/ssf/Documents/Github/runlayer && npx jest --no-coverage 2>&1 | tail -20
```
...
cd /home/ssf/Documents/Github/runlayer && npx tsc --noEmit 2>&1 | head -20
```
...
- Added \`ModelTier\` and \`AiCompleteRequestInput\` type exports to \`runlayer/src/contracts/ai-complete.contract.ts\`
