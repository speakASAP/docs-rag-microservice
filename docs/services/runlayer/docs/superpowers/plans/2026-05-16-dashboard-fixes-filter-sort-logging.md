...
Run:
```bash
grep -c "async function loadTasksSection" /home/ssf/Documents/Github/runlayer/public/app.js
```
Expected output: `1` (exactly one definition). If you see `2`, delete the old one.

- [ ] **Step 4: Manual smoke test**

Navigate to `https://runlayer.alfares.cz/tasks` in the browser. Verify:
...
The legacy URL pattern `https://runlayer.alfares.cz/#/projects/flipflop-v1/tasks` uses a hash prefix. The `initAuth()` function already strips `#/` prefixes via `history.replaceState`. The fix is to ensure the page is deployed with updated cache-busting so browsers load the latest `app.js`.
...
Run:
```bash
grep -A5 "hash.startsWith" /home/ssf/Documents/Github/runlayer/public/app.js
```
...
Run:
```bash
grep -A6 "taskMatch" /home/ssf/Documents/Github/runlayer/public/app.js | head -12
```
...
```bash
grep -A5 "goBackToTasks" /home/ssf/Documents/Github/runlayer/public/app.js | head -10
```
...
```
Expected: `deployment "runlayer" successfully rolled out`

- [ ] **Step 3: Smoke test production**

1. Open `https://runlayer.alfares.cz/tasks` — verify filter bar is visible
2. Open `https://runlayer.alfares.cz/#/tasks` — verify URL becomes `https://runlayer.alfares.cz/tasks` automatically (hash stripped)
