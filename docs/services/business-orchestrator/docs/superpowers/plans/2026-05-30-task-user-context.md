...
```bash
find /home/ssf/Documents/Github/business-orchestrator/src -name "*Migration*" -o -name "*migration*" | grep -v node_modules | head -10
```
...
npx typeorm migration:run -d src/app.module.ts 2>&1 || \
  psql "$DATABASE_URL" -c "ALTER TABLE business_orchestrator.tasks ADD COLUMN IF NOT EXISTS user_context JSONB NOT NULL DEFAULT '{\"distilled\":\"\",\"thread\":[]}';"
```
...
kubectl exec -n statex-apps deploy/database-server -- psql -U postgres -d postgres -c \
  "ALTER TABLE business_orchestrator.tasks ADD COLUMN IF NOT EXISTS user_context JSONB NOT NULL DEFAULT '{\"distilled\":\"\",\"thread\":[]}';"
```
...
```bash
kubectl exec -n statex-apps deploy/database-server -- psql -U postgres -d postgres -c \
  "SELECT column_name, data_type FROM information_schema.columns WHERE table_name='tasks' AND table_schema='business_orchestrator' AND column_name='user_context';"
```
...
```bash
ls /home/ssf/Documents/Github/business-orchestrator/public/
```
...
```bash
kubectl exec -n statex-apps deploy/database-server -- psql -U postgres -d postgres -c \
  "SELECT column_name FROM information_schema.columns WHERE table_name='tasks' AND table_schema='business_orchestrator' AND column_name='user_context';"
```
...
```bash
# Add a comment
curl -s -X POST https://orchestrator.alfares.cz/api/dashboard/tasks/<TASK_ID>/comment \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text":"test comment from smoke test"}' | jq '.task.userContext'

# Patch priority
curl -s -X PATCH https://orchestrator.alfares.cz/api/dashboard/tasks/<TASK_ID> \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
Navigate to `https://orchestrator.alfares.cz/tasks`, click "Details" on any task.
