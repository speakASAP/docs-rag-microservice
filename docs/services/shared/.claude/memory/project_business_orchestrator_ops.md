---
name: Business Orchestrator Operational Facts
description: Operational gotchas for business-orchestrator: auth, endpoints, agent seeding, deploy tag collision, coordinator trigger
type: project
originSessionId: 97bfe054-af9a-4643-8724-d69dfed69d9b
---
Operational facts for the business-orchestrator K8s service (statex-apps namespace).
...
To observe coordinator activity: `kubectl logs -n statex-apps deployment/business-orchestrator --since=6m | grep -i "coordinator\|cycle\|goal\|task_created"`
...
```sql
INSERT INTO business_orchestrator.agents (id, type, status, model_tier, capabilities, last_heartbeat, created_at)
VALUES (gen_random_uuid(), 'worker', 'idle', 'free', '[]', NOW(), NOW());
```
Run 3× for concurrency. Check: `SELECT id, type, status FROM business_orchestrator.agents;`
...
```bash
TAG=$(date +%Y%m%d%H%M)
docker build -t localhost:5000/business-orchestrator:$TAG .
docker push localhost:5000/business-orchestrator:$TAG
kubectl set image deployment/business-orchestrator business-orchestrator=localhost:5000/business-orchestrator:$TAG -n statex-apps
...
Database: `business_orchestrator` on `db-server-postgres` in `statex-apps`. Tables are in schema `business_orchestrator` (not `public`): `business_orchestrator.goals`, `business_orchestrator.tasks`, etc.
...
Insert a goal:
```sql
INSERT INTO business_orchestrator.goals (id, project_id, title, status, constraints, created_at, updated_at)
...
- `target_service`: string — the microservice directory name under `CODING_AGENT_REPO_ROOT` (e.g. `'business-orchestrator'`)
- `smoke_test_urls`: string[] — HTTP URLs returning 200 after deploy (e.g. `['http://business-orchestrator:3390/health']`)
